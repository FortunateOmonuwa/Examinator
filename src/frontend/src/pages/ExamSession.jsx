...
const ExamResults = () => {
  ...
  const isAnswerCorrect = (question, questionIndex, userAnswer) => {
    ...
    if (question.type === "SINGLECHOICE") {
      ...
      return (
        correctOptionIndices.length === userAnswerArray.length &&
        correctOptionIndices.every((idx) => userAnswerArray.includes(idx))
      );
  ...
  const getUserAnswerText = (question, userAnswer) => {
    if (question.type === "TEXT") {
      ...
      if (userAnswer !== null && userAnswer !== undefined) {
        return question.options[userAnswer]?.text || "No answer selected";
      }
      return "No answer selected";
    } else if (question.type === "MULTICHOICE") {
      if (userAnswer && userAnswer.length > 0) {
        return userAnswer.map((idx) => question.options[idx]?.text).join(", ");
      }
