  ...
  if (!exam) {
    return (
      <div className="text-center py-12">
        <h3 className="mt-2 text-lg font-medium text-gray-900">Exam not found</h3>
        <p className="mt-1 text-sm text-gray-500">The exam you're looking for doesn't exist.</p>
        <div className="mt-6">
          <Link
            to="/"
    ...
    <div className="view-exam">
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-6 exam-details">
        <div className="px-4 py-5 sm:px-6">
          ...
          <div className="mt-2">
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 subject-tag">
              {exam.subject}
            </span>
          </div>
        </div>
          ...
          <div key={question.id} className="bg-white shadow overflow-hidden sm:rounded-lg p-6 question-card">
            <div className="mb-4">
              <h3 className="text-lg font-medium text-gray-900">
                Question {qIndex + 1}
                {question.required && <span className="ml-2 text-sm text-red-500">*Required</span>}
              </h3>
            ...
            <div className="space-y-3 mt-4 options-list">
              ...
              {question.options.map((option, oIndex) => (
                <div key={option.id} className="flex items-center option-item">
                  ...
                  >
                    {option.isCorrect && (
                      <span className="text-white text-xs flex items-center justify-center w-full">✓</span>
                    )}
                  </div>
